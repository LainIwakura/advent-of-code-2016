...
main() ->
    Pass = lists:keysort(2, decode_pw([], [], "uqwqemis", 0)),
    Decoded = lists:map(fun(X) -> {D,_} = X, D end, Pass),
    io:format("~p~n", [Decoded]).

decode_pw(Pw, _, _, _) when length(Pw) =:= 8 ->
    Pw;
decode_pw(Pw, Seen, In, Idx) ->
    Hash = bin_to_hex(crypto:hash(md5, In ++ integer_to_list(Idx))),
    case Hash of
        <<$0,$0,$0,$0,$0,Index:8,Digit:8,_Rest/binary>> when Index =< $7 -> 
            case lists:member(Index,Seen) of 
                true -> decode_pw(Pw, Seen, In, Idx+1); 
                false -> decode_pw([{Digit, Index-48}|Pw], [Index|Seen], In, Idx+1) 
            end;
        _ -> decode_pw(Pw, Seen, In, Idx+1)
    end.

%% Below is a very fast bin-to-hex solution from stack overflow.
%% Without this it'd take upwards of 10 seconds to decode 1 digit
